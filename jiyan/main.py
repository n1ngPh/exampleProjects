...
from jiyan3_float import ResultException
...
class Geetest_Slide(JiyanTrack):
    def __init__(self):
        ...
        self.api_slide = "http://localhost:3000/get_w"
    ...
    def get_s_c(self, gt, challenge):
        ...
        return response_dict["data"]["s"], response_dict["data"]["c"]

    def get_geetest_ajax_user(self, gt, challenge):
        try:
            params = {
                "gt": gt,
                "challenge": challenge,
    ...
    def get_material(self, gt, challenge):
        """
        获取验证素材信息
        @param gt:
        @param challenge:
        ...
        try:
            ...
            data = re.search(r"geetest_\d+(.*)", response.text).group(1)
            data_dict = eval(data.replace("true", "'true'").replace("false", "'false'"))
            logger.debug("获取验证素材信息成功")
            gt = data_dict["gt"]
            s = data_dict["s"]
            challenge_2 = data_dict["challenge"]
            id = data_dict["id"]
            ...
            return gt, challenge_2, s, id, gap_bg, full_bg, gct_path
        except Exception as e:
            logger.error(f"参数解析失败 -> {response.text}")
            raise e
    ...
    def get_validate(self, gt, challenge_2, s, c, distance, passtimes, track):
        try:
            api_payload = {
                "gt": gt,
            ...
            res_api = self.session.post(url=self.api_slide, json=api_payload).json()
            if res_api["msg"] == "success":
                ...
                response_dict = re.findall(r'\((.+?)\)', response_dict)[0]
                response_dict = json.loads(response_dict)
    ...
    def main(self):
        ...
        # 第一个get.php
        s, c = self.get_s_c(gt, challenge)
        # 第一个ajax.php
        self.get_geetest_ajax_user(gt, challenge)
        # 第二个get.php
        gt_new, challenge_2, s_s, id, gap_bg, full_bg, gct_path = self.get_material(gt, challenge)
        # 模拟验证距离、轨迹、时间
        distance, track, passtimes = self.get_distance_track(self.session, self.host + gap_bg, self.host + full_bg)
        # 第二个ajax.php
        validate = self.get_validate(gt_new, challenge_2, s_s, c, distance, passtimes, track)
